...
import { Search, Tag, X, Target, RefreshCw } from "lucide-react";
...
const SmartSheet = () => {
  ...
  const [sheetLastUpdated, setSheetLastUpdated] = useState(null);
  ...
  useEffect(() => {
    const fun = async () => {
      const result = (await getFullSheet())
      console.log("Full sheet : ", result);
      setAllProblems(result?.sheet || []);
      setPersonalized(result?.personalized ?? false);
      setSheetLastUpdated(result?.lastUpdated ?? null);
    }
    if (allProblems) {
  ...
  const rebuildSheet = async () => {
    setRebuilding(true);
    try {
      const result = await getFullSheet(true);
      setAllProblems(result?.sheet || []);
      setPersonalized(result?.personalized ?? false);
      setSheetLastUpdated(result?.lastUpdated ?? null);
      setCurrentPage(1);
  ...
  return (
    pageLoading ? (
      ...
      >
        ...
        {personalized ? (
          <>
            <p className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide px-3 py-1 rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 mb-4">
              <Target className="w-3.5 h-3.5" />
              Generated from your AI Report
            </p>
            <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
              <span className="font-bold">Refined set of 20 problems</span>, algorithmically weighted and filtered down to specifically target the weak areas your <span className="font-bold">AI Report</span> flagged.
            </p>
          </>
        ) : (
          <p className="text-lg text-gray-600 dark:text-gray-300 max-w-2xl mx-auto">
            Showing generally popular problems for now — generate your <span className="font-bold">AI Report</span> on the Dashboard so this sheet can be refined down to <span className="font-bold">your specific weak areas</span>.
          </p>
        )}
        ...
        <div className="flex items-center justify-center gap-3 mt-4 flex-wrap">
          {sheetLastUpdated && (
            <span className="text-xs text-gray-500 dark:text-gray-400">
              Last generated {new Date(sheetLastUpdated).toLocaleString()}
            </span>
          )}
          <button
            onClick={rebuildSheet}
            disabled={rebuilding}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${rebuilding ? 'animate-spin' : ''}`} />
            {rebuilding ? 'Rebuilding...' : 'Rebuild sheet'}
          </button>
        </div>
      </div>
      ...
      >
        {filteredProblems.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 dark:text-gray-400">Try adjusting your filters, or regenerate your AI Report on the Dashboard for a fresh set of weak-topic problems.</p>
          </div>
        ) : (
          <>
                      ...
                      <div className="flex flex-col gap-4">
                            ...
                            <div className="flex flex-col lg:flex-row items-end lg:items-center gap-3">
                              <div className="flex items-center gap-2">
                                <span className="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-400 px-3 py-2 rounded-lg text-lg font-bold">
                                  {problem.connectionsSolved}
                                </span>
                                <span className="text-sm font-medium text-gray-600 dark:text-gray-400 whitespace-nowrap">connections solved</span>
                              </div>
                        ...
                        <div className="flex sm:hidden items-center justify-between gap-3 pt-2 border-t border-gray-200 dark:border-gray-700">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium text-gray-600 dark:text-gray-400">connections solved</span>
                          </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {showTags && (
                            <div className="flex flex-wrap gap-2 justify-end items-center">
                              {problem.matchedTopic && (
                                <span
                                  className="px-2 py-1 bg-indigo-600 text-white rounded text-xs sm:text-sm font-medium whitespace-nowrap flex items-center gap-1"
                                  title="Weak topic from your AI report this problem targets"
                                >
                              ...
                              {problem.matchedTag && (
                                <span
                                  className="px-2 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800 rounded text-xs sm:text-sm font-mono whitespace-nowrap"
                                  title="Exact tag matched from your practice data"
                                >
                                  {problem.matchedTag}
                                </span>
                              )}
                              {problem.task.tags.filter(tag => tag !== problem.matchedTag).map((tag, tagIndex) => (
                                <span
                                  key={tagIndex}
